...
- **Pinned and verified.** `flutter.toolchain(flutter_version = "...")`
  ...
  Omitting `flutter_version` selects the newest stable entry in that checked-in
  table; this default is deterministic and advances only when the version
  updater rewrites the table.
...
| `{name}.dev`, `flutter_widget_preview`, `{name}.update`, `build_runner` run helpers | The Flutter SDK they invoke (sealed and pinned; the `flutter`-driven `.dev`/preview/`.update` helpers additionally set `FLUTTER_ALREADY_LOCKED` + `--no-version-check`, while the build_runner helpers invoke `dart` directly and rely on the seal). | These are `bazel run` helpers that operate on your **source** workspace by design: the dev server serves live sources; Widget Preview creates and updates `.widget_preview` and may resolve its generated scaffold through the network and user pub cache; `.update` re-resolves dependencies. `.update` works in a temporary copy and writes back only `pubspec.lock`; preview `clean` removes only the package-local `.widget_preview`. | Developer-loop tooling, intentionally outside the build graph. |
