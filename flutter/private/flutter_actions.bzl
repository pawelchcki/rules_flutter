...
def flutter_build_action(
        ...
        linux_setup_file = None,
        linux_tool_launcher_file = None,
        linux_cmake_initial_cache_file = None):
    ...
    Args:
        ...
        linux: Hermetic compiler/build-tool/GTK package closure for linux.
        ...
        linux_setup_file: Private shell helper that prepares the Linux action
            environment and its action-local tool wrappers.
        linux_tool_launcher_file: Private shell helper used by those wrappers.
        linux_cmake_initial_cache_file: Private CMake cache initialized for
...
""".format(web_normalizer = web_normalizer_file.path)
    ...
    # --no-pub` assumes it already exists. Native targets therefore run an
    ...
    native_pub_get = ""
    if target in ANDROID_TARGETS or target in ["ios", "macos", "linux", "windows"]:
        native_pub_get = """
...
"""
    ...
    if target == "linux":
        if linux == None:
            fail("flutter_app '{}' target 'linux' requires a Linux toolchain.".format(ctx.label))
        if linux_setup_file == None or linux_tool_launcher_file == None or linux_cmake_initial_cache_file == None:
            fail("flutter_build_action: linux target requires private Linux setup helpers")
        linux_env = """
# Every Linux build prerequisite lives below this declared package closure.
LINUX_ROOT="$ORIGINAL_PWD/{root}"
LINUX_LOADER="$LINUX_ROOT/{loader}"
LINUX_LIB_PATH="$LINUX_ROOT/lib/{triple}:$LINUX_ROOT/usr/lib/{triple}:$LINUX_ROOT/usr/lib/llvm-14/lib:$LINUX_ROOT/lib:$LINUX_ROOT/usr/lib"
LINUX_TOOL_BIN="$BUILD_WORKSPACE_TMP/.rules_flutter_linux_bin"
LINUX_LINKER_SCRIPTS="$BUILD_WORKSPACE_TMP/.rules_flutter_linux_linker_scripts"
LINUX_GNU_TRIPLE="{triple}"
LINUX_TOOL_LAUNCHER="$ORIGINAL_PWD/{linux_tool_launcher}"
LINUX_CMAKE_INITIAL_CACHE="$ORIGINAL_PWD/{linux_cmake_initial_cache}"
source "$ORIGINAL_PWD/{linux_setup}"
""".format(
            loader = linux.loader,
            linux_cmake_initial_cache = linux_cmake_initial_cache_file.path,
            linux_setup = linux_setup_file.path,
            linux_tool_launcher = linux_tool_launcher_file.path,
            root = linux.root,
            triple = linux.gnu_triple,
        )
...
""".format(
    ...
    inputs = depset(
        direct = [working_dir, pub_cache_dir, dart_tool_dir, pub_tool_file] +
                 ...
                 ([linux_setup_file, linux_tool_launcher_file, linux_cmake_initial_cache_file] if target == "linux" else []) +
                 flutter_toolchain.flutterinfo.tool_files,
        transitive = [_sdk_files(flutter_toolchain, target)] +
                     ([android.files] if android else []) +
                     ([linux.files] if linux else []),
    )
